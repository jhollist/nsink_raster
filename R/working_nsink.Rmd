...
niantic_stream_removal <- niantic_streams %>%
  ...
  mutate(totma = case_when(totma == -9999 ~ NA_real_,
                           TRUE ~ totma)) %>%
  mutate(n_removal = 1 - exp(-0.0513 * (mean_reach_depth^-1.319) * totma)) %>%
...
niantic_lake_removal <- niantic_waterbodies %>%
  ...
  mutate(meandused = case_when(meandused < 0 ~ NA_real_,
                               TRUE ~ meandused)) %>%
  mutate(n_removal = 79.24 - (33.26 * log10(meandused/lake_residence_time_yrs))) %>%
...
# Merge rasters (not yet working)

```{r}
niantic_n_removal <- raster::merge(niantic_lake_removal, niantic_stream_removal, niantic_hydric_removal)
```
