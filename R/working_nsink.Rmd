...
6. Extract and combine together in combo flowpath vector with removal

```{r}
niantic_fp <- rbind(fp_ends[1,],select(fp_flowlines, geometry)) %>%
  mutate(value = 1)  
  # Trying to figure out how to get this as a single feature for the extract
...
#JEFF!!! Look at the list here.  Might solve a lot of the in and out issues.  
#Also think about filtering the removal raster to remove the small issues first.
#July 3 - did this - generate removal layer at 10m, focal max filter with 3x3,
#Then rescale back to 30m.  Lot less empty cells from rasterization mis-match.
niantic_fp_removal <- raster::extract(niantic_fp_removal_r, niantic_fp,along = TRUE)
#Need to convert to work with stack
# try extract stack, then unlist to type and removal check lengths
#fp_removal <- lapply(niantic_fp_removal[-1],function(x) Mode(x[,1]))
#fp_removal_first <- zoo::rollapply(niantic_fp_removal[1][[1]][,1], width = 2, partial = TRUE, 
#                          function(x) ifelse(x[1]==x[2],0, x[1]))
#fp_types <- lapply(niantic_fp_removal[-1],function(x) Mode(x[,2]))
fp_removal <- unlist(niantic_fp_removal)
fp_removal <- fp_removal[!is.na(fp_removal)]
```

7. Vector is Corrector ...
...
```{r}
fp_removal_max_filter <- zoo::rollmax(fp_removal, 3)
```
...
```{r}
#Need to get working with stack
niantic_fp_types <- raster::extract(niantic_types, niantic_fp, along = TRUE)
fp_types <- unlist(niantic_fp_types)
fp_types <- fp_types[!is.na(fp_types)]
fp_types_max_filter <- zoo::rollmax(fp_types, 3)
...
output_1 <- type_removal_df %>%
  mutate(n_in = round(cumprod(c(100,1-n_removal))[-n],2),
         ...
         percent_removal = round(n_removal*100,2)) %>%
...
niantic_fp_r <- niantic_fp %>%
  raster::rasterize(empty_raster_30,
                    ...
                    max)
niantic_fp_removal_r <- niantic_fp_r * niantic_n_removal
x <- raster::extract(niantic_fp_removal_r, niantic_fp, along = T)
mapview(niantic_fp_removal_r, niantic_fp)
```
