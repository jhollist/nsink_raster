...
nhd_vaa <- read.dbf(here("data/NHDPlusNE/NHDPlus01/NHDPlusAttributes/PlusFlowlineVAA.dbf")) #joined above somewhere..
...
niantic_lake_flow_path_removal <- niantic_streams %>%
  left_join(nhd_vaa, by = c("COMID" = "ComID")) %>%
  left_join(nhd_lakemorpho, by = c("WBAREACOMI" = "ComID")) %>%
  left_join(nhd_lakemorpho_flowline, by = c("COMID" = "ComID")) %>%
  filter(!is.na(MeanDepth)) %>%
  filter(WBAreaType == "LakePond") %>% 
  group_by(WBAREACOMI) %>%
  mutate(lake_residence_time_yrs = sum(TOTMA*0.002737851)) %>%
  ungroup() %>%
  mutate(removal = 79.24 - (33.26 * log10(MeanDUsed/lake_residence_time_yrs))) %>%
  select(COMID, WBAREACOMI, removal, lake_residence_time_yrs)
View(niantic_lake_flow_path_removal)
