...
# Get artifical flow path removal table
niantic_stream_sf 
%>%
  ...
  ungroup() %>%
  mutate(n_removal = case_when(ftype == "ArtificalPath" ~ 79.24 - (33.26 * log10(meandused/lake_residence_time_yrs)/as.numeric(st_length(.))*30),
                               TRUE ~ n_removal))
  
  
  
  
  mutate(n_removal = case_when(is.infinite(n_removal) ~ 0,
                               TRUE ~ n_removal)) %>%
  as("Spatial")
