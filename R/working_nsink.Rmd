...
niantic_hydric <- st_as_sf(niantic_ssurgo$spatial) %>%
  ...
  left_join(niantic_ssurgo$tabular$component, by = "mukey") %>%
  select(mukey, hydricrating) %>%
  filter(hydricrating == "Yes") #not quite.  Doesn't look quite right...
```
...
st_read(here("data/gSSURGO_CT.gdb","MUPOLYGON"))
