...
```{r}
land_removal <- raster::extract(niantic_n_removal, fp_ends[1,], along = TRUE)
not_land_removal <- raster::extract(niantic_n_removal, fp_flowlines, along = TRUE)
fp_removal <- unlist(c(land_removal, not_land_removal))
fp_removal <- fp_removal[!is.na(fp_removal)]
```

7. Total Reduction Calculation

```{r}
fp_last <- zoo::rollapply(fp_removal, width = 2, partial = TRUE, 
                          function(x) ifelse(x[1]==x[2],0, x[1]))
fp_last <- ifelse(is.na(fp_last),fp_removal[length(fp_removal)],fp_last)
...
Note: 2018-05-24
- order of vector from raster is N to South and west to east, not along flow path
- flow accumulation could be used to order vector...
- enough slop in raster conversion that still have some cases where cell or two pops out of stream or lake or... Can result in a single removal type (e.g a lake) would be double counted using rollapply.  Smaller resolution on flow path???  Larger window on rollapply??
