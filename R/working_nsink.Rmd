...
if(!file.exists(here("data/nhdplus_ne_fdrnull.7z"))){
  fdr_null_url <- "http://www.horizon-systems.com/NHDPlusData/NHDPlusV21/Data/NHDPlusNE/NHDPlusV21_NE_01_01a_FdrNull_01.7z"
  httr::GET(fdr_null_url,httr::write_disk(path = here("data/nhdplus_ne_fdrnull.7z")),progress())
  archive_extract(here("data/nhdplus_ne_fdrnull.7z"), here("data/"))
}
...
ne_fdr_null <- raster(here("data/NHDPlusNE/NHDPlus01/NHDPlusFdrNull01a/fdrnull/"))
niantic_fdr_null <- crop(fdr_ne, as(niantic_huc,"Spatial"))
...
Ideas:
1. Calc flow path
2. Get all intersecting stream flow lines.
3. Find most downstream and upstream flow line and grab all flowlines in between
4. Get portion of flow path above stream flowlines
5. Get portion of flow path below strea flwolines (it happens, happens a lot)
...
fp_n <- st_sfc(st_linestring(example_fp_xy), crs = st_crs(niantic_streams))
stream_fp<-niantic_streams %>%
  slice(st_intersects(fp_n,niantic_streams)[[1]])

fp_n_b <- st_sf(st_buffer(fp_n, 15)) %>% 
  fasterize::fasterize(raster = empty_raster, field = NULL)
#mapview(fp_n_b) + niantic_streams


plot(niantic_n_removal)
plot(fp, add = T)
plot(niantic_streams, add = T)
